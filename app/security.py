...
def current_merchant(
    ...
    """Return the ``sub`` claim from a compact HMAC-SHA256 JWT."""
    ...
    try:
        ...
        expected = _b64encode(hmac.new(_jwt_secret, signed, hashlib.sha256).digest())
        if not hmac.compare_digest(signature, expected):
            raise ValueError("invalid token")
        claims = json.loads(_b64decode(payload))
        expires_at = claims.get("exp")
        if isinstance(expires_at, (int, float)) and expires_at < _utc_now().timestamp():
            raise ValueError("invalid token")
